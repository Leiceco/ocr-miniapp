...
- **智能分类**：文本描述自动提取金额、判断收支、匹配分类（14 类内置关键词，支持自定义）
- **语音录入**：语音转文字 + 自动解析，边说边记账
- **账本推荐**：根据近期使用频率自动推荐最合适的账本
- **异常检测**：疑似重复、金额突增、预算超支/预警，保存时即时提醒
- **每日记账提醒**：本地定时推送（时间可自定义），自动检查空账本
- **智能通知**：根据用户活跃时段动态调整提醒时间，降低打扰感
- **周报/月报**：自动生成消费报告（分类排名 + 趋势对比），可推送通知
- **完全离线**：所有智能功能均在设备内运行，零 API 调用，数据不上传
...
│   │   ├── bill_parser.dart            # 智能解析：规则引擎提取金额/分类/类型
│   │   ├── anomaly_detector.dart       # 异常检测：重复/突增/超预算校验
│   │   ├── book_recommender.dart       # 账本推荐：按使用频率自动推荐
│   │   ├── notification_service.dart   # 通知服务：定时提醒+智能时机调整
│   │   └── report_service.dart         # 报告服务：周报/月报生成与推送
│   └── utils/
...
### v2.0.0 (2026-06-25) — AI 智能记账
- ✨ 智能分类：文本描述自动提取金额、判断收支、匹配分类（14 类内置关键词库）
- ✨ 语音录入：语音转文字 + 自动解析，边说边记账，解放双手
- ✨ 账本推荐：根据近 30 天使用频率自动推荐最合适的账本
- ✨ 异常检测：保存时自动校验疑似重复、金额突增（超 3 倍均值）、预算超支/预警
- ✨ 每日记账提醒：本地定时推送（默认 21:00），自动检查当天是否有记录
- ✨ 智能通知时机：根据用户活跃时段（8-23 点）动态调整提醒时间
- ✨ 快捷补录：点击通知直接进入 App
- ✨ 周报/月报：自动统计消费趋势，生成分类排名和同比变化摘要
- ✨ 报告推送：周日自动推送周报，每月 1 号推送月报
- ✨ 提醒时间设置：设置页新增 🔔 通知设置，用户可自定义提醒时间
- 🔧 完全离线：所有智能功能（解析/推荐/检测/报告）均在设备内运行，零 API 调用

### v1.1.0 (2026-06-25) — 数据导入导出
